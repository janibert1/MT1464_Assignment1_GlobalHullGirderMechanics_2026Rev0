...
def solve_q1(digits):
    ...
    draw_multiplot_svg('q1a_g_plots.svg', x,
    ...
    draw_multiplot_svg('q1f_compare.svg', x,
                       [Fs, Fs_n, Mb, Mb_n],
    ...
    # Derivative checks away from discontinuities (segment steps, crane point load representation, moonpool edges)
    ...
    with open('check_q1a_f.md', 'w') as f:
...
def solve_q2(digits):
    ...
    # Simplified section from Fig. 4 assumptions:
    # - Deck and bottom: thickness 4*tp over full breadth B
    # - 4 vertical plates (2 side shells + 2 longitudinal bulkheads): thickness 2*tp over full depth H
    t_h = 4 * tp
    t_v = 2 * tp
    ...
    # Horizontal elements (deck + bottom)
    A_h = B * t_h
    I_h_own = B * t_h ** 3 / 12
    d_h = H / 2 - t_h / 2
    I_h_shift = A_h * d_h ** 2
    ...
    with open('answer_q2a_f.md', 'w') as f:
...
def main(studienummer="6470114"):
    ...
    solve_q1(digits)
    solve_q2(digits)
    print('Generated: answer_q1a_g.md, check_q1a_f.md, q1a_g_data.csv, q1a_g_plots.svg, q1f_compare.svg, answer_q2a_f.md')
